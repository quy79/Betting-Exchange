...
namespace BetEx247.Data.DAL.Sports
{
    ...
    public partial class SoccerCorrectScoresService : ISoccerCorrectScoresService
    {
        ...
        /// <returns></returns>
        public List<Soccer_CorrectScores> SoccerCorrectScoreses(long sportID, long countryID, long LeagueID, string matchID)
        {
            using (var dba = new BetEXDataContainer())
            {
                var list = dba.Soccer_CorrectScores.Where(w => w.MatchID == matchID & w.SportID == sportID & w.CountryID == countryID & w.LeagueID == LeagueID).ToList();
