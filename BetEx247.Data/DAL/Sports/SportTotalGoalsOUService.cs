...
namespace BetEx247.Data.DAL.Sports
{
    public partial class SportTotalGoalsOUService
    {
        ...
        /// <returns></returns>
        public List<Sports_TotalOU> SportTotalGoalsOUs(long sportID, long countryID, long LeagueID, Guid matchID)
        {
            using (var dba = new BetEXDataContainer())
            {
                var list = dba.Sports_TotalOU.Where(w => w.ID == matchID & w.SportID == sportID & w.CountryID == countryID & w.LeagueID == LeagueID).ToList();
