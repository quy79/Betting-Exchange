...
namespace BetEx247.Data.DAL.Sports
{
    public partial class SoccerMatchOddsService : ISoccerMatchOddsService
    {
        ...
        /// <returns></returns>
        public List<Soccer_MatchOdds> SoccerMatchOddses(long sportID, long countryID, long LeagueID, string matchID)
        {
            using (var dba = new BetEXDataContainer())
            {
                var list = dba.Soccer_MatchOdds.Where(w => w.MatchID == matchID & w.SportID == sportID & w.CountryID == countryID & w.LeagueID == LeagueID).ToList();
